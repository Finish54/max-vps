...
from bot.misc.callbackData import (
    ...
    ChooseProtocolList, ChoosingTypeServer, ShowMetric, RemoveMetric,
    ServerAutoWork, StatusUserPanel
)
...
async def choosing_auth_method(lang) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(
        text=_('server_auth_token_btn', lang),
        callback_data=ChoosingAuthMethod(method='token')
    )
    kb.button(
        text=_('server_auth_login_btn', lang),
        callback_data=ChoosingAuthMethod(method='login')
    )
    kb.adjust(2)
    return kb.as_markup()


async def choosing_sub_path(lang) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(
        text=_('server_sub_path_skip_btn', lang),
        callback_data='sub_path_skip'
    )
    kb.adjust(1)
    return kb.as_markup()
